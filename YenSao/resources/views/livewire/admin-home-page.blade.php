        ...
        <div class=" rounded-md p-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class=" w-full bg-gray-800 rounded-lg p-10">
                    ...
                    <div class=" mb-5">
                        <select id="chartype"
                            class=" text-center w-20 h-8 rounded-sm border border-gray-100 bg-gray-800 text-white font-bold">
                ...
                <table class=" w-full text-white">
                    ...
                    @foreach ($TotalProduct as $index => $product)
                        ...
                        <tr>
                            ...
                            <td>{{ $product->name }}</td>
                            <td>
                                <div class=" bg-black/25 w-40 rounded-md h-2 mx-auto">
                                    <div style="width: {{ $percant }}%"
                                        class=" rounded-md h-full {{ $bgColor }}"></div>
                                </div>
                            </td>
                            <td>
                                <div
                                    class=" h-10  {{ $bgColor }} bg-opacity-25 rounded-sm border {{ $borderColor }} font-bold
                             {{ $textColor }} mx-auto flex justify-center object-center pt-2">
                                    {{ $percant }}%</div>
                            </td>
                        </tr>
                    @endforeach
                        ...
                        @if ($editable == $ship->id)
                            <tr>
                                <form>
                                    ...
                                    <td>
                                        <input wire:model.debounce.200ms="name" value="{{ $name }}"
                                            type="text"
                                            class="bg-gray-800 text-white border border-gray-700 rounded-lg p-1 w-32 focus:ring-2 focus:ring-gray-500 focus:outline-none">
                                        @error('name')
                                            <div class="bg-red-500 text-sm text-black">{{ $message }}</div>
                                        @enderror
                                    </td>
                                    <td><input wire:model.debounce.200ms="email" value="{{ $email }}"
                                            type="text"
                                            class="bg-gray-800 text-white border border-gray-700  rounded-lg p-1 w-full focus:ring-2 focus:ring-gray-500 focus:outline-none">
                                        @error('email')
                                            <div class="bg-red-500 text-sm text-black">{{ $message }}</div>
                                        @enderror
                                    </td>
                                    <td>
                                        <input wire:model.debounce.200ms="phone" value="{{ $phone }}"
                                            type="text"
                                            class="bg-gray-800 text-white border border-gray-700  rounded-lg p-1 w-full focus:ring-2 focus:ring-gray-500 focus:outline-none">
                                        @error('phone')
                                            <div class="bg-red-500 text-sm text-black">{{ $message }}</div>
                                    ...
                                    <td><input wire:model.debounce.200ms="address" value="{{ $address }}"
                                            type="text"
                                            class="  bg-gray-800 text-white border border-gray-700  rounded-lg p-1 w-72 focus:ring-2 focus:ring-gray-500 focus:outline-none">
                                        @error('address')
                                            <div class="bg-red-500 text-sm text-black">{{ $message }}</div>
                                        @enderror
                                    </td>
                                    <td><input wire:model.debounce.200ms="cost" value="{{ $cost }}"
                                            type="text"
                                            class="bg-gray-800 text-white border border-gray-700  rounded-lg p-1 w-full focus:ring-2 focus:ring-gray-500 focus:outline-none">
                                        @error('cost')
                                            <div class="bg-red-500 text-sm text-black">{{ $message }}</div>
                                    ...
                                    <td>
                                        <select wire:model.live='status'
                                            class="bg-black/25 text-white h-10 p-2 border-none outline-none cursor-pointer custom-select">
                                            <option value="0" class="bg-gray-800 text-red-500 font-bold">Chưa giao
                                            </option>
                                            <option value="1" class="bg-gray-800 text-yellow-500 font-bold">Đang
                                                giao</option>
                                            <option value="2" class="bg-gray-800   text-green-500 font-bold">Đã
                                                giao</option>
                                        </select>
                                    </td>
                                    <td class="text-white">
                                        <button wire:loading.attr='disable' wire:click='EditAble({{ $ship }})'><i
                                                class="fa-solid fa-ban mr-2"></i></button>
                                        <button wire:loading.attr='disable' wire:click='UpdateShip({{ $ship }})'><i
                        ...
                        @else
                            <tr class="text-white">
                                ...
                                @else
                                    ...
                                    @else
                                        <td class="text-green-500 font-bold">Đã giao</td>
                                    @endif
                                @endif
                                <td>
                                    <button wire:loading.attr='disable' wire:click='EditAble({{ $ship }})'>
                                        <i class="fa-solid fa-pencil  mr-2"></i></button>
                                    <button wire:loading.attr='disable' wire:click='DeleteShip({{ $ship }})'><i
        ...
        data: {
            ...
            datasets: [
...

                {
                    ...
                    data: [1, 2, 25, 55, 65, 75, 35, 12, 20, 30, 40, 30],
